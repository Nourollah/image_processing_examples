...
im = np.array(cv2.imread("4.jpg", cv2.IMREAD_GRAYSCALE))
# Plot original image
plt.subplot(131)
...
b = np.array([0.0, 0.15, 0.7, 0.8, 1.0])
...
plt.subplot(133)
plt.plot(a, b)
out = np.array(cv2.normalize(out.astype('float32'), None, 0, 256, cv2.NORM_MINMAX)).reshape(im.shape)
plt.subplot(132)
